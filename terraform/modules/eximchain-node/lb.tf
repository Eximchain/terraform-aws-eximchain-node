...
resource "aws_lb" "eximchain_node" {
  ...
  security_groups = [aws_security_group.eximchain_load_balancer[0].id]
}
...
resource "aws_lb_listener" "eximchain_node_rpc" {
  ...
  load_balancer_arn = aws_lb.eximchain_node[0].arn
  port              = 22000
  protocol          = "HTTP"

  default_action {
    target_group_arn = aws_lb_target_group.eximchain_node_rpc[0].arn
    type             = "forward"
  }
}
...
resource "aws_security_group_rule" "eximchain_load_balancer_rpc_self" {
  ...
  security_group_id = aws_security_group.eximchain_load_balancer[0].id
  type              = "ingress"
